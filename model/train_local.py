...
def setup_logging(log_dir: Path, run_name: str) -> logging.Logger:
    ...
    logger = logging.getLogger("diabetes_train_local")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Train a local diabetes MLP and log metrics/weights.")
    ...
    ap.add_argument("--out-dir", type=str, default="runs/diabetes_mlp_local", help="Base output directory.")
    ap.add_argument("--run-name", type=str, default=None, help="Run name (defaults to timestamped name).")
    ap.add_argument("--log-weight-stats", action="store_true", help="Log per-tensor weight stats each epoch (sample).")
    ...
    ap.add_argument("--save-best-ckpt", action="store_true", help="Save best model checkpoint to disk.")
    args = ap.parse_args()
    ...
    # Run naming + directories (run-specific folder so you can compare later)
    if args.run_name is None:
        from datetime import datetime
        args.run_name = f"run_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

    base_out = Path(args.out_dir)
    run_dir = base_out / args.run_name
    ckpt_dir = run_dir / "checkpoints"
    log_dir = run_dir / "logs"
    stats_dir = run_dir / "weight_stats"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_loss < best_val - 1e-6:
            ...
            if args.save_best_ckpt:
                ...
                torch.save(
                    {"epoch": epoch, "best_val": best_val, "state_dict": best_state},
                    ckpt_dir / "best.pt",
                )
                logger.info(f"Saved best checkpoint: {ckpt_dir / 'best.pt'}")
    ...
    logger.info(f"Saved training history: {run_dir / 'history.json'}")
    ...
    logger.info(f"Saved metrics: {run_dir / 'metrics.json'}")
